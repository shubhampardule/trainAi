...
| `safetensors` | The export format. `trainai export` writes `model.safetensors`, so a shared model has no pickle in its loading path at all — not a restricted one, none. Checkpoints are *not* safetensors: they are a single `torch.save` payload holding optimizer and RNG state as well as weights, read with `weights_only=True`, which restricts the unpickler but still relies on torch getting that right. Sharing goes through the export, and that difference is the path this dependency protects; the reasoning is in [checkpoint-format.md](../checkpoint-format.md). |
